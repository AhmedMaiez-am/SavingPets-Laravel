            ...
            <div class="content cover text-center">
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        <h1 class="mt-0 mb-4">Event informations</h1>

                        <div class="card">
                            <!-- <div class="card-header">Associations informations</div> -->
                            <div class="card-body">
                                <div class="card-body">
                                    <h4 class="card-title">Nom de donneur : {{ $donations->donatorName }}</h4>
                                    <h4 class="card-text">Type : {{ $donations->type }}</h4>
                                    <h4 class="card-text">Prix : {{ $donations->amount }}</h4>
                                    <h4 class="card-text">Type : {{ $donations->type }}</h4>
                                    <h4 class="card-text">Evènement : {{ $donations->event_id }}</h4>
                        ...
                        <a href="{{ url('/donations/' . $donations->id . '/edit') }}" title="Edit donations"><button
                                class="btn btn-green"><i class="fa fa-pencil-square-o" aria-hidden="true"></i>
                                Edit</button></a>
                        <form method="POST" action="{{ url('/donations' . '/' . $donations->id) }}"
                            accept-charset="UTF-8" style="display:inline">
                            {{ method_field('DELETE') }}
                            {{ csrf_field() }}
                            <button type="submit" class="btn btn-green" title="Delete donations"
