...
<div id="navbar-top">
    ...
    <section id="showA" class="page overlay">
        <div class="container">
            <div class="content cover text-center">
                <div class="row">
                    ...
                    <h1 class="mt-0 mb-4" >Association informations</h1>

                     <div class="card" >
           <!-- <div class="card-header">Associations informations</div> -->
            <div class="card-body">
              <div class="card-body" >
                <h4 class="card-title">titre : {{ $associations->titre }}</h4>
                <h4 class="card-text">adresse : {{ $associations->adresse }}</h4>
                <h4 class="card-text">telephone : {{ $associations->telephone }}</h4>
                <h4 class="card-text">email : {{ $associations->email }}</h4>
       ...
       <a href="{{ url('/associations/' . $associations->id . '/edit') }}" title="Edit associations"><button class="btn btn-green"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit</button></a>
        <form method="POST" action="{{ url('/associations' . '/' . $associations->id) }}" accept-charset="UTF-8" style="display:inline">
                                                {{ method_field('DELETE') }}
                                                {{ csrf_field() }}
         <button type="submit" class="btn btn-green" title="Delete associations" onclick="return confirm(&quot;Confirm delete?&quot;)"><i class="fa fa-trash-o" aria-hidden="true"></i> Delete</button>
       </form>
